...
<form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('patients/patient_summery_d');?>">
            ...
            <div class="panel-body" style="font-size: 11px;">
             ...
             <h4 style="margin-top: 5px;margin-bottom: 5px;"><p class="text-center"><?php echo $this->session->userdata('address') ?></p></h4>
                    ...
                    <?php
                        ...
                        ?>
                        <h3 style="margin-top: 0px; margin-bottom: 15px;"><?php if($name) { echo "Departmental ";} else { echo "OPD DURGS DISTRIBUTION"; }?>  <?php  if($name) { echo "(".$name.")";}?></h3>
...
<table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
    ...
    foreach($department_new as $dept_new){
        $male_new = $this->db->select('count(*) as male_new')->from('patient')->where('ipd_patient_flag', '0')->where('department_id', $dept_new->dprt_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'M')->where('yearly_reg_no !=', '')->get()->row();
        $male_old = $this->db->select('count(*) as male_old')->from('patient')->where('ipd_patient_flag', '0')->where('department_id', $dept_new->dprt_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'M')->where('old_reg_no !=', '')->get()->row();
        $female_new = $this->db->select('count(*) as female_new')->from('patient')->where('ipd_patient_flag', '0')->where('department_id', $dept_new->dprt_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'F')->where('yearly_reg_no !=', '')->get()->row();
        $female_old = $this->db->select('count(*) as female_old')->from('patient')->where('ipd_patient_flag', '0')->where('department_id', $dept_new->dprt_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'F')->where('old_reg_no !=', '')->get()->row();
