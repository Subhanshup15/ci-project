<div class="row">
  ...
  { $table='patient_ipd'; }
				  					$name = $this->db->select('*')->from($table)->where('id',$haematology_pro->name)->get()->row();
  									if($name)
                                    {
                                      echo $name->firstname;
                                    }else {
  									echo $haematology_pro->name; } ?>
                                  </strong></div>
                                <div class="col-xs-6"><strong> Date:</strong> <strong><?php echo date('d-m-Y',strtotime($haematology_pro->create_date)); ?></strong></div>
                            </div>
                            <?php if($section == 'ipd'){ ?>
                            <div class="row" style="margin-bottom: 5px;">
                                <div class="col-xs-6"><strong>Gender:</strong> <?php if($haematology_profile_ipd[0]->sex == 'M'){ echo "Male"; } else{ echo"Female"; }?></strong></div>
                                <div class="col-xs-6"><strong>Age:</strong> <?php echo $haematology_profile_ipd[0]->date_of_birth; ?></strong></div>
                            </div>
                            <?php } else{ ?>
                             <div class="row" style="margin-bottom: 5px;">
                                <div class="col-xs-6"><strong>Gender:</strong> <?php if($haematology_profile_opd[0]->sex == 'M'){ echo "Male"; } else{ echo"Female"; }?></strong></div>
                                <div class="col-xs-6"><strong>Age:</strong> <?php echo $haematology_profile_opd[0]->date_of_birth; ?></strong></div>
                            </div>
                            <?php } ?>
                        </div>
