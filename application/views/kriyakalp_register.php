...
<div class="row">
   <!--  table area -->
   <div class="col-sm-12">
      <form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('patients/kriyakalp_register')?>">
      ...
      <div  class="panel panel-default thumbnail">
         ...
         <div class="panel-heading  row" id="PrintMe">
            ...
            <div class="col-lg-12" style="text-align:center;margin-top: 10px;margin-bottom: 10px;">
                        <?php  if($section == 'ipd') { ?> 
                                <h2> IPD Kriyakalp Register</h2>
                    <?php }else  {?>
                    <h2> OPD Kriyakalp Register</h2>
                    <?php }?>
            </div>
            <div class="col-lg-12" style="text-align:center;margin-top: 10px;margin-bottom: 10px;">
               <div>
                  ...
                  <thead>
                     <tr>
                       ...
                       <th rowspan='2'><?php echo "Date" ?></th>
                        ...
                        <th rowspan='2'><?php echo "Kriyakalp" ?></th>
  ...
  if($tretment && is_object($tretment)){
    $SHIROBASTI = isset($tretment->SHIROBASTI) ? $tretment->SHIROBASTI : null;
    $OTHER = isset($tretment->OTHER) ? $tretment->OTHER : null;
    $NASYA = isset($tretment->NASYA) ? $tretment->NASYA : null;

  					if((stripos($OTHER, 'tarpan') !== false) || 
                       (stripos($OTHER, 'KARNA DHOOPAN') !== false) || 
                       (stripos($OTHER, 'GANDUSH') !== false) ||
                       ($NASYA) ||
                       ($SHIROBASTI))
                    {
                    ?>
                     <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                        ...
                        <td><?php if($patient->yearly_reg_no !=''){ echo $patient->yearly_reg_no.'/'.$Year;  } ?></td>
                       ...
                       if($tretment->NASYA)
                            {
                              echo  $NASYA= $tretment->NASYA.'<br>'; 
                            }
                       if($tretment->SHIROBASTI)
                            {
                              echo  $SHIROBASTI = $tretment->SHIROBASTI.'<br>'; 
                            }
                       if($tretment->OTHER)
                            {
                              echo  $OTHER = $tretment->OTHER.'<br>'; 
                            }
                          ?></td>
                        <?php 
...
// Fetch the department details with the year condition
