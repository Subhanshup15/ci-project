         ...
         <div class="panel-body panel-form">
           ...
           <?//php echo form_open_multipart('patients/save_pharma_patient_count','class="form-inner"') ?>
            ...
            <div class="row">
                <div class="col-md-6">
                    <lable>ECG</lable>
                    <input type="text" name="ECG" id="ECG" placeholder="ECG" class="form-control" readonly>
                </div>
...
$(document).ready(function() {
    $('body').on('keyup change', '#date', function() {
        ...
        $.ajax({
            ...
            url     : '<?php echo base_url('patients/fetch_patient_ecg') ?>',
    ...
    $('body').on('keyup change', '#patient_name', function() {
        ...
        $.ajax({
            ...
            url     : '<?php echo base_url('patients/fetch_treatment_ecg') ?>',
            ...
            success : function(data) {
                if (data.status == true) {
                   $('#tests').empty();
                    ...
                    if(ECG){
                    $('#tests').append(
                                         '<input type="test" name="ECG_final" id="ECG_final" readonly="readonly" value="' + ECG + '"><br><br>'
                            );
                    }
                    if(ECG){
                    $('#tests').append(
                                        //  '<input type="text" name="xray_result" id="xray_result"><br>'
                                         '<textarea id="ecg_result" placeholder="Add Result" name="ecg_result" rows="4" cols="50">'
                            );
                    }
...
$("#submit").click(function(){
        ...
        var ECG = $('#ECG_final').val();
        var ecg_result = $('#ecg_result').val();
        ...
        $.ajax({
            ...
            url:'<?php echo base_url('patients/save_ecg_data') ?>',
