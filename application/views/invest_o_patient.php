      ...
      <div  class="panel panel-default thumbnail">
         ...
         <div class="panel-body panel-form">
           ...
           <?php echo form_open_multipart('patients/save_pharma_patient_count','class="form-inner"') ?>
            ...
            <div class="row">
                <div class="col-md-6">
                    <lable>HEMATOLOGICAL</lable>
                    <input type="text" name="HEMATOLOGICAL" id="HEMATOLOGICAL" placeholder="HEMATOLOGICAL" class="form-control" readonly>
                </div>
                <div class="col-md-6">
                    <lable>SEROLOGYCAL</lable>
                    <input type="text" name="SEROLOGYCAL" id="SEROLOGYCAL" placeholder="SEROLOGYCAL" class="form-control" readonly>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <lable>BIOCHEMICAL</lable>
                    <input type="text" name="BIOCHEMICAL" id="BIOCHEMICAL" placeholder="BIOCHEMICAL" class="form-control" readonly>
                </div>
                <div class="col-md-6">
                    <lable>MICROBIOLOGICAL</lable>
                    <input type="text" name="MICROBIOLOGICAL" id="MICROBIOLOGICAL" placeholder="MICROBIOLOGICAL" class="form-control" readonly>
                </div>
            </div>
...
<div class="container">
    ...
    <div class="modal-dialog" style="">
      <!-- Modal content-->
      <div class="modal-content" style="width: 1260px;margin-left:-200px">
                 ...
                 <div>
                     ...
                     <table class="table table-bordered">
                         <thead>
                             <tr>
                               <th>Sr.No.</th>
                               <th id="testnameh">Test Name</th>
                               <th id="result1h" >Result</th>
                               <th id="unith">Unit</th>
                               <th id="test_type1h">Reference Range</th>
                               <th id ="wd20h">1:20</th>
                               <th id ="wd40h">1:40</th>
                               <th id ="wd80h">1:80</th>
                               <th id ="wd160h">1:160</th>
                               <th id ="wd320h">1:320</th>
                             </tr>
                         </thead>
                         <tbody>
                             <tr>
...
$(document).ready(function() {
    ...
    $('body').on('keyup change', '#date', function() {
        ...
        $.ajax({
            ...
            url     : '<?php echo base_url('patients/fetch_patient_investi') ?>',
    ...
    $('body').on('keyup change', '#patient_name', function() {
        ...
        $.ajax({
            ...
            url     : '<?php echo base_url('patients/fetch_treatment_investi') ?>',
            ...
            success : function(data) {
                if (data.status == true) {
                    ...
                    $('#tests').empty();
                    $('#tests1').empty();
                    var hm = data.HEMATOLOGICAL;
                    if(hm){
                      
                     var result = hm.split(",");
                    //  alert(result);
                    }
                    var se = data.SEROLOGYCAL;
                    if(se)
                    {
                    var result1 = se.split(",");
                    //   alert(result1);
                    }
                  
                    var bi = data.BIOCHEMICAL;
                    if(bi)
                    {
                    var result2 = bi.split(",");
                          // alert(result2);
                    }
                  
                    var mi = data.MICROBIOLOGICAL;
                    if(mi)
                    {
                    var result3 = mi.split(",");
                        //   alert(result3);
                    }
                    
                   
                    
                    $('#HEMATOLOGICAL').val(data.HEMATOLOGICAL);
                    $('#SEROLOGYCAL').val(data.SEROLOGYCAL);
                    $('#BIOCHEMICAL').val(data.BIOCHEMICAL);
                    $('#MICROBIOLOGICAL').val(data.MICROBIOLOGICAL);
                    ...
                    $('#create_date').val(data.create_date);
                        ...
                        if(hm)
                        {
                          ...
                          {
                          	$('#tests').append('<input type="radio" name="test_type" id="test_type" value="' + result[i] + '">'+ result[i] +'<br>');
                          }
                        }
                      if(se)
                      {
                        ...
                        {
                          $('#tests').append('<input type="radio" name="test_type" id="test_type" value="' + result1[j] + '">'+ result1[j] +'<br>');
                        }
                      }
                      if(bi)
                      {
                        ...
                        {
                          $('#tests').append('<input type="radio" name="test_type" id="test_type" value="' + result2[k] + '">'+ result2[k] +'<br>');
                        }
                      }
                      ...
                      if(mi)
                      {
                        ...
                        {
                          $('#tests').append('<input type="radio" name="test_type" id="test_type" value="'+ result3[l] +'">'+ result3[l] +'<br>');
                        }
                      }
                } else {
                    $(".invlid_patient_id").text('<?php echo display("invalid_patient_id") ?>');
...
$(document).ready(function() {
      ...
      $('body').on('keyup change', '#test_type', function() {
        ...
        console.log($('#test_type').val());
        
        if($("input[name='test_type']:checked").val() == 'WIDAL TEST')
        {
          $("#testnameh").show();
          $("#result1h").hide();
          $("#unith").hide();
      ...
      //  alert(name);
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                {
                  console.log(newarray[j].test_name);
                  var test_name = newarray[j].test_name;
                   var html = "<input type='text' name='testname' style='margin-top: 3px;' id='testname'  value="+ newarray[j].test_name +" readonly='readonly'>";
                   //var html = "<input type='text' name='testname' style='margin-top: 3px;' id='testname' readonly='readonly' value='BLOOD SUGER RANDOM(f)'>";
                ...
                {
                   var html = "<input type='text' style='margin-top: 3px;' name='wd20d1' id='wd20d1'>";
                    $('#wd20d').append(html);
                }
                  
                    for(var q=0;q<newarray.length;q++)
                {
                   var html = "<input type='text' style='margin-top: 3px;' name='wd40d1' id='wd40d1'>";
                    $('#wd40d').append(html);
                }
                    for(var q=0;q<newarray.length;q++)
                {
                   var html = "<input type='text' style='margin-top: 3px;' name='wd80d1' id='wd80d1'>";
                    $('#wd80d').append(html);
                }
                    for(var q=0;q<newarray.length;q++)
                {
                   var html = "<input type='text' style='margin-top: 3px;' name='wd160d1' id='wd160d1'>";
                    $('#wd160d').append(html);
                }
                    for(var q=0;q<newarray.length;q++)
                {
                   var html = "<input type='text' style='margin-top: 3px;' name='wd320d1' id='wd320d1'>";
                    $('#wd320d').append(html);
                }
...
$("#submit").click(function(){
        ...
        var section = $('#section_final').val();
        var name = $('#name_final').val();
        var patient_auto_id = $('#patient_id_final').val();
        var dignosis = $('#dignosis_final').val();
        var create_date = $('#create_date_final').val();
        var time = $('#time').val();
  
  
  
   var wd20d1 =[];
        $("input[name='wd20d1']").val(function(){
            wd20d1.push(this.value);
        });
   var wd40d1 =[];
        $("input[name='wd40d1']").val(function(){
            wd40d1.push(this.value);
        });
   var wd80d1 =[];
        $("input[name='wd80d1']").val(function(){
            wd80d1.push(this.value);
        });
   var wd160d1 =[];
        $("input[name='wd160d1']").val(function(){
            wd160d1.push(this.value);
        });
   var wd320d1 =[];
        $("input[name='wd320d1']").val(function(){
            wd320d1.push(this.value);
        });
        
        var testname =[];
        $("input[name='testname']").val(function(){
            testname.push(this.value);
        });
  
        
        var result1 =[];
        $("input[name='result']").val(function(){
            result1.push(this.value);
        });
        
        var unit =[];
        $("input[name='unit']").val(function(){
            unit.push(this.value);
        });
        
        var referencerange =[];
        $("input[name='reference_range']").val(function(){
            referencerange.push(this.value);
        });
        
         var report_type =[];
        $("input[name='report_type']").val(function(){
            report_type.push(this.value);
        });
        
        var report_section =[];
        $("input[name='report_section']").val(function(){
            report_section.push(this.value);
        });
        
        var test_type1 =[];
        $("input[name='test_type1']").val(function(){
            test_type1.push(this.value);
        });
        $.ajax({

            url:'<?php echo base_url('patients/save_investi_data') ?>',
               ...
               success:function(data) {
                ...
                if (data.status == true) { 
                    ...
                    $('#submit').submit(function(e) {
                        var html = "Investigation Saved Successfully!";
                    $('#save_success').append(html);
                         e.preventDefault();
                        $('#myModal').modal('hide');
                         var html = "Investigation Saved Successfully!";
                         $('#save_success').append(html);
                    ...
                    $(".save_failed").text('<?php echo display("Somthing Wents Wrong!") ?>');
                }
            },
            error:function() {
              //  alert('failed!');
