	...
	<?php  echo error_reporting(0);?>
		...
		<div class="row">
    		<div class="col-sm-12">
				...
				<form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('patients/major_ot'); ?>">
            ...
            <?php foreach($department as $dept){ ?>
                <option value="<?php echo $dept->dprt_id;?>"><?php echo $dept->name;?></option>
            <?php } ?>
            </select>
            </div>  
                    <div class="form-group">
             ...
             <h4 style="margin-top: 5px;margin-bottom: 5px;"><p class="text-center"><?php echo $this->session->userdata('address') ?></p></h4>
                    ...
                    <?php   
                     ...
                     ?>
                        ...
                        <h3 style="margin-top: 0px; margin-bottom: 15px;"><?php  echo "Major Operation Theater Register";?></h3>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}?> rowspan="2"><?php echo "Address" ?></th> 
                              <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}?> rowspan="2"><?php echo "Department" ?></th>          
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}?> rowspan="2"><?php echo "DOA" ?></th>
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}?> rowspan="2"><?php echo "OT Date" ?></th>
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}?> rowspan="2"><?php echo "DOD" ?></th>
                            <th style="width: 30px; text-align: center;"><?php echo "Mojor OT" ?></th>
			...
			?>
                            ...
                            //atya
                              ...
                              //st
                                ...
                                $datefrom1=date('Y-m-d',strtotime($patient->create_date));
                                ...
                                {
                                    $doctor_name= $this->db->select("*")->from('user')
                                      				->where('join_date <=', $datefrom1) 
                                                    ->where('department_id', $patient->department_id) 
                                      				->order_by("user_id", "DESC")
                                                    ->get()
                    ...
                    //    $dept_name = $this->db->select('*')->from('department')->where('dprt_id', $patient->department_id)->get()->row();                      
                              ?>
                                ...
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>" style="  <?php if(($date_c==$date_f) && ($ipd == 'ipd')){ echo "color: #ff000d;font-weight: bold;"; } else if(($date_d==$date_f) && ($ipd == 'ipd')) { echo "color: #4dd208;font-weight: bold;" ;} else if(($New_OPD ==$patient->yearly_reg_no) && ($old_OPD == $patient->old_reg_no) && ($ipd == 'opd')){ echo "color: #ff000d;font-weight: bold;"; } else { echo ""; } ?>">
                                    ...
                                    <td>
                                        <?php  
                                            if(stripos($patient->dignosis, 'ARSHA') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'PARIKARTIKA') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'BHAGANDAR') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'PERIANAL ABSCESS') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'PILONIDLE SINUS') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'PHYMOSIS') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'RAKTARSHA') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'SENTINAL TAG') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'BHAGANDAR') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'MUTRAVRUDDHI') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'HYDROCELE') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'VATAJ ARSHA') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'NIRUDDHA PRAKASHA') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'PILONIDLE SINUS') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'INTERNAL HAEMORRHOIDS') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'ACUTE FISSURE IN ANO') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'TYMPANIC MEMBRANE PERFORATION') !== false){ echo $patient->dignosis; }
                                            elseif(stripos($patient->dignosis, 'GILAYU SHOPH') !== false){ echo $patient->dignosis; }
                                             elseif(stripos($patient->dignosis, 'PTERYGIUM') !== false){ echo $patient->dignosis; }
                                        ?>
                                    </td> 
                                  
                                    <td style="width: 30px;"><?php echo $skarma;?></td> 
