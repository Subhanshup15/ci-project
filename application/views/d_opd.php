...
<form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('Patient_New/dopd_date');?>">
                ...
                <table width="100%" id="patientdata"  class="table table-striped table-bordered table-hover" <?php  if($gob=='gob') { echo "style='font-size:10px;'";}?> style="display:  <?php  if($summery_report == 1) { echo "none";}?>">
                    <thead>
                        <tr>
                             ...
                             <?php if($ipd == 'ipd'){ ?> <th style="width: 30px;" rowspan="2"><?php echo "S.No."; ?></th><?php } ?> 
                          <!--   <?php if($ipd == 'opd'){ ?> <th style="width: 30px;" rowspan="2"><?php echo "Yearly No."; ?></th><?php } ?> -->
                            ...
                            $sixty_above_all_mn=0;$sixty_above_all_mo=0;$sixty_above_all_fn=0;$sixty_above_all_fo=0;
                             ...
                             $dd12=date('Y-m-d', strtotime($_GET['start_date']));
                                ...
                                $dd1=date('Y-m-d', strtotime($_GET['start_date']));
                                 ...
                                 $dd1=date('Y-m-d');
                                    ...
                                    $age_ranges = [
                                        ...
                                        $department_ids = ['36','35', '34', '33', '32', '31', '30', '29', '28', '27'];
                              ...
                              ?>
                               ...
                               <?php if($ipd==ipd){   ?>   <td  style="padding:2px;"><?php echo $i;  ?></td> <?php } ?>
                                    <?php } else {?>
                             ...
                             <?php if($ipd == 'ipd'){ ?>  <td  style="padding:2px;"><?php  if($tretment->POVISIONALdignosis) { echo $tretment->POVISIONALdignosis; }else { echo $p_dignosis_name; } ?></td> <?php } ?> 
                                    ...
                                    <td  style="padding:2px;"><?php  echo $doctor_name->firstname;?></td><?php } ?>
                                        ...
                                        <?php }?>
                                                                ...
                                                                <?php
                                                                $panch_patient_from_panchkarma_table_2025 = $this->db->select('*')->from('panchkarma_patient_count_ipd')->where('create_date', $datefrom)->where('patient_auto_id', $patient->id)->get()->row();

                                                                if ($panch_patient_from_panchkarma_table_2025) {
                                                                ?>
                                                                <td style="padding:2px;">

                                                                <?php if ($panch_patient_from_panchkarma_table_2025->snehan) { echo $panch_patient_from_panchkarma_table_2025->snehan . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->swedan) { echo $panch_patient_from_panchkarma_table_2025->swedan . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->vaman) { echo $panch_patient_from_panchkarma_table_2025->vaman . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->virechan) { echo $panch_patient_from_panchkarma_table_2025->virechan . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->basti) { echo $panch_patient_from_panchkarma_table_2025->basti . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->nasya) { echo $panch_patient_from_panchkarma_table_2025->nasya . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->raktmokshan) { echo $panch_patient_from_panchkarma_table_2025->raktmokshan . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->shirodhara) { echo $panch_patient_from_panchkarma_table_2025->shirodhara . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->shirobasti) { echo $panch_patient_from_panchkarma_table_2025->shirobasti . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->others) { echo $panch_patient_from_panchkarma_table_2025->others . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->yonidhavan) { echo $panch_patient_from_panchkarma_table_2025->yonidhavan . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->yonipichu) { echo $panch_patient_from_panchkarma_table_2025->yonipichu . ', <br>'; } ?>
                                                                <?php if ($panch_patient_from_panchkarma_table_2025->uttarbasti) { echo $panch_patient_from_panchkarma_table_2025->uttarbasti . ', <br>'; } ?>
                                                                </td>
                                                                <?php
                                                                } else {
                                                                ?>
...
?>
                ...
                <!-- Table Summery -->
                  ...
                  <h3>Report Summary</h3>
                              ...
                              <?php 
$department_new =  $this->db->select('*')->from('department_new')->where('dprt_id',$dept_id)->get()->row();
?>
<?php if($ipd == 'opd'){?>
<table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
    ...
    $total_female_old = 0;
        ...
        $male_new = $this->db->select('count(*) as male_new')->from('patient')->where('department_id', $dept_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'M')->where('yearly_reg_no !=', '')->get()->row();
        $male_old = $this->db->select('count(*) as male_old')->from('patient')->where('department_id', $dept_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'M')->where('old_reg_no !=', '')->get()->row();
        $female_new = $this->db->select('count(*) as female_new')->from('patient')->where('department_id', $dept_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'F')->where('yearly_reg_no !=', '')->get()->row();
        $female_old = $this->db->select('count(*) as female_old')->from('patient')->where('department_id', $dept_id)->where('create_date', $datefrom)->where('ipd_opd', 'opd')->where('sex', 'F')->where('old_reg_no !=', '')->get()->row();
