...
<div class="row">
    ...
    <form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('patients/get_seriology_patients')?>">
        ...
        <div  class="panel panel-default thumbnail">
            ...
            <div class="panel-heading  row" id="PrintMe">
                ...
                <div class="col-lg-12" style="text-align:center;margin-top: 10px;margin-bottom: 10px;">
                    ...
                    if($section == 'ipd'){
                    ?>
                    <h2>Serology Investigation IPD</h2>
                    <?php } else { ?>
                    <h2>Serology Investigation OPD</h2>
                    <? } ?>
                </div>
...
?>
                                ...
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                                    ...
                                    <td>
                                      ...
                                      if($section=='opd')
                                                                 ...
                                                                 { $table='patient_ipd'; }                  
                                                                 $name = $this->db->select('*')->from($table)->where('id',$slg->patient_name)->get()->row();
                                                                 if($name)
                                                                 {
                                                                   echo $name->firstname;
                                                                 }else {
                                                                   echo $slg->patient_name; }
                                      ?></td>
                                    ...
                                    <td>
                                        <?php echo $slg->serology; ?>
                                    </td>      
                                    <td class="center no-print">
                                        <a href="<?php echo base_url("patients/get_seriology_patient_profile/$slg->patient_auto_id/$section") ?>" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></a> 
                                        <a href="<?php echo base_url("laboratory/editbiochemical/$slg->patient_auto_id") ?>" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a> 
                                        <!--<a href="<?php echo base_url("laboratory/deletebiochemical/$slg->patient_auto_id") ?>" class="btn btn-xs btn-danger" onclick="return confirm('<?php echo display('are_you_sure') ?>')"><i class="fa fa-trash"></i></a> -->
                                    </td>                                     
                                </tr>
                                <?php $sl++;?>
                            <?php } ?> 
                   ...
                   <table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php 
                            $name_array=array('SEROLOGYCAL'); 
                            $tot_array=array($investiResultCountTest->serologyCount);
                        ?> 
                        <?php $n = 1; ?>
