...
<div class="row">
    <div class="col-sm-12">
<form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('Patient_New/patient_by_date_occupancy'); ?>">
			                ...
			                ->row();
			                 ...
			                 $dept_name= $dept_name->name;
                               ...
                               $ipd = ($patients[0]->ipd_opd);
                                ...
                                if($ipd == 'ipd'){ ?>
                    <h3 style="margin-top: 0px; margin-bottom: 15px;">Bed Occupancy Patient Register</h3>
                ...
                <table width="100%" id="patientdata" class="table table-striped table-bordered table-hover" <?php  if($gob=='gob') { echo "style='font-size:10px;'";}?>>
                    <thead>
                        <tr>
                            ...
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}else{echo "style='width:10px;'";}?> rowspan="2"><?php echo display('sex') ?></th>   
                            <th  <?php  if($gob=='gob') { echo "style='width:1px;'";}else{echo "style='width:10px;'";}?> rowspan="2" ><?php echo "Age" ?></th>                  
                          ...
                          <!--  <?php if($department_by !='dpt') {?><th style="width: 30px;" rowspan="2"><?php if($ipd == 'ipd'){ echo "Doctor Name" ;} else { }?></th> <?php } ?>  -->
                         
                           
                          <?php if($ipd=='ipd'){ if($department_by !='dpt'){ ?> <th style="96px" rowspan="2"><?php echo "Date"?></th><?php } ?><?php }?> 
                           
                         <?php if($ipd=='ipd'){ if($department_by !='dpt'){ ?> <th style="width: 30px;" rowspan="2"><?php echo "Bed No";?></th><?php } ?><?php }?>
                          
                           
                    
                        
                            <th class="no-print" rowspan="2" style="width:20px;"><?php echo display('action') ?></th> 
...
?>
                            ...
                            $skym_mn=0;$skym_mo=0;$skym_fn=0;$skym_fo=0; $skym_tt=0;
                             ...
                             $dd12=date('Y-m-d', strtotime($_GET['start_date']));
                              if($_GET['start_date']){
                                 ...
                                 $dd1=date('Y-m-d');
                                    ...
                                    $departments = [
                                        '27' => ['vs_mn', 'vs_mo', 'vs_fn', 'vs_fo', 'vs_tt'],
                                        '28' => ['sw_mn', 'sw_mo', 'sw_fn', 'sw_fo', 'sw_tt'],
                                        '29' => ['st_mn', 'st_mo', 'st_fn', 'st_fo', 'st_tt'],
                                        '30' => ['sky_mn', 'sky_mo', 'sky_fn', 'sky_fo', 'sky_tt'],
                                        '31' => ['sly_mn', 'sly_mo', 'sly_fn', 'sly_fo', 'sly_tt'],
                                        '32' => ['ba_mn', 'ba_mo', 'ba_fn', 'ba_fo', 'ba_tt'],
                                        '33' => ['pn_mn', 'pn_mo', 'pn_fn', 'pn_fo', 'pn_tt'],
                                        '34' => ['ky_mn', 'ky_mo', 'ky_fn', 'ky_fo', 'ky_tt'],
                                        '35' => ['aa_mn', 'aa_mo', 'aa_fn', 'aa_fo', 'aa_tt'],
                                        '36' => ['skym_mn', 'skym_mo', 'skym_fn', 'skym_fo', 'skym_tt'],
                                    ];

                                    if (array_key_exists($patient->department_id, $departments)) {
                                        list($mn, $mo, $fn, $fo, $tt) = $departments[$patient->department_id];
                                        if ($patient->sex == 'M' && $patient->yearly_reg_no) {
                                            if ($dd != $dd1) { $$mn++; }
                                        }
                                        if ($patient->sex == 'M' && $patient->old_reg_no) {
                                            $$mo++;
                                        }
                                        if ($patient->sex == 'F' && $patient->yearly_reg_no) {
                                            if ($dd != $dd1) { $$fn++; }
                                        }
                                        if ($patient->sex == 'F' && $patient->old_reg_no) {
                                            if ($dd != $dd1) { $$fo++; }
                                        }
                                        if ($dd != $dd1) {
                                            $$tt++;
                              ...
                              ?>
                                ...
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>" style="  <?php if(($date_c==$date_f) && ($ipd == 'ipd')){ echo "color: #ff000d;font-weight: bold;"; } else if(($date_d==$date_f) && ($ipd == 'ipd')) { echo "" ;} else { echo ""; } ?>">
                                  ...
                                  <!-- <td  style="padding:2px;"><?php 
                                    ...
                                    if($ipd == 'ipd'){ echo $doctor_name->firstname;} else { }?></td> --><?php } ?>
                                     
                                     <?php if($department_by !='dpt') {?> <td  style="padding:2px; width:96px;"><?php echo "DOA :".substr($patient->create_date,0,10);?></td> <?php } ?> 
                                      
                                      <?php if($ipd=='ipd'){?><?php if($department_by !='dpt') {?> <td  style="padding:2px;"><?php echo $patient->bedNo;?></td> <?php } ?><?php } ?>
                ...
                <table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
                    ...
                    <?php $n=1;
                             if($ipd=='ipd')
                    {
                              $male_new=array($skym_mn,$ky_mn,$pn_mn,$ba_mn,$sly_mn,$sky_mn,$st_mn,$vs_mn);
                              $male_old=array($skym_mo,$ky_mo,$pn_mo,$ba_mo,$sly_mo,$sky_mo,$st_mo,$vs_mo);
                              $female_new=array($skym_fn,$ky_fn,$pn_fn,$ba_fn,$sly_fn,$sky_fn,$st_fn,$vs_fn);
                              $female_old=array($skym_fo,$ky_fo,$pn_fo,$ba_fo,$sly_fo,$sky_fo,$st_fo,$vs_fo);
                              $total=array($skym_tt,$ky_tt,$pn_tt,$ba_tt,$sly_tt,$sky_tt,$st_tt,$vs_tt);
                              }
                              else
                              {
