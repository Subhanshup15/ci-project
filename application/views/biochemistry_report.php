...
<div class="row">
    ...
    <form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('patients/get_biochemistry_patients')?>">
        ...
        <div  class="panel panel-default thumbnail">
            ...
            <div class="panel-heading  row" id="PrintMe">
                ...
                <div class="col-lg-12" style="text-align:center;margin-top: 10px;margin-bottom: 10px;">
                    ...
                    if($section == 'ipd'){
                    ?>
                    <h2>Biochemistry Investigation IPD</h2>
                    <?php } else { ?>
                    <h2>Biochemistry Investigation OPD</h2>
                    <? } ?>
                </div>
            ...
            <div class="panel-body">
                <table width="100%" id="patientdata" class=" table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php if (!empty($biochemistry)) {
                   //     print_r($haematology);
                         // $test = $haematology->hematology;
                        ?>
                            <?php $sl = 1;
?>
                            <?php foreach ($biochemistry as $bcm) {  ?>
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                                    <!-- <td><?php //echo $patient->patient_id; ?></td> // Yearly reg no -->
                                    <td><?php echo $sl; ?></td>
                                    <td><?php echo $bcm->yearly_reg_no; ?></td> 
                                    <td><?php echo $bcm->old_reg_no; ?></td> 
                                    <td><?php if($section=='opd')
                                                                 { $table='patient'; }
                                                                 else 
                                                                 { $table='patient_ipd'; }                  
                                                                 $name = $this->db->select('*')->from($table)->where('id',$bcm->patient_name)->get()->row();
                                                                 if($name)
                                                                 {
                                                                   echo $name->firstname;
                                                                 }else {
                                                                   echo $bcm->patient_name; } ?></td>
                                    ...
                                    <td>
                                     ...
                                     <td>
                                        <?php echo $bcm->biochemistry; ?>
                                    </td>      
                                    <td class="center no-print">
                                        <a href="<?php echo base_url("patients/get_biochemistry_patient_profile/$bcm->patient_auto_id/$section") ?>" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></a> 
                   ...
                   <table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php 
                            $name_array=array('BIOCHEMICAL'); 
                            $tot_array=array($investiResultCountTest->biochemistryCount);
                        ?> 
                        <?php $n = 1; ?>
