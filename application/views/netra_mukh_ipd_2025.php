...
<form class="form-inline" id="datefilter" name="datefilter" method="GET" action="<?php echo base_url('Patient_New/netra_mukh_report_ipd'); ?>">
...
<div class="form-group">
  ...
  <label class="sr-only" for="end_date"><?php echo display('end_date') ?></label>
    ...
    <input type="hidden" name="department_id" class="form-control" id="department_id" value="<?php echo $department_id; ?>">
     <input type="hidden" name="section" class="form-control" id="section" value="<?php echo $section; ?>">
               ...
               <?php  $department_new =  $this->db->select('*')->from('department_new')->where('dprt_id',$department_id)->get()->row(); ?>
                <table width="100%" id="patientdata" class="table table-striped table-bordered table-hover">
                    ...
                    $total_female_old = 0;
                        ...
                        $male_new = $this->db->select('count(*) as male_new')->from('patient_ipd')->where('department_id', $department_id)->where('create_date', $start_date)->where('ipd_opd', 'opd')->where('sex', 'M')->where('yearly_reg_no !=', '')->get()->row();
                        // print_r($this->db->last_query());
                        $male_old = $this->db->select('count(*) as male_old')->from('patient_ipd')->where('department_id', $department_id)->where('create_date', $start_date)->where('ipd_opd', 'opd')->where('sex', 'M')->where('old_reg_no !=', '')->get()->row();
                        $female_new = $this->db->select('count(*) as female_new')->from('patient_ipd')->where('department_id', $department_id)->where('create_date', $start_date)->where('ipd_opd', 'opd')->where('sex', 'F')->where('yearly_reg_no !=', '')->get()->row();
                        $female_old = $this->db->select('count(*) as female_old')->from('patient_ipd')->where('department_id', $department_id)->where('create_date', $start_date)->where('ipd_opd', 'opd')->where('sex', 'F')->where('old_reg_no !=', '')->get()->row();
