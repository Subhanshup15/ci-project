    ...
    */
    ...
    public function check_patient($mode = null)
    {
        ...
        if (!empty($patient_id)) {
            $query = $this->db->select('firstname,lastname')
                ->from('patient')
                ->where('yearly_reg_no', $patient_id)
                ->where('status', 1)
                ->get();
    ...
    public function doctor_by_department()
    {
        ...
        if (!empty($department_id)) {
            ...
            $option1 = "<option value=\"\">" . display('select_option') . "</option>";
            if (($query->num_rows() > 0) || ($query1->num_rows() > 0)) {
                ...
                foreach ($query1->result() as $bed) {
                    $option1 .= "<option value=\"$bed->id\">$bed->id</option>";
                }

                $data['message'] = $option;
                $data['message1'] = $option1;
                $data['status'] = true;
            } else {
