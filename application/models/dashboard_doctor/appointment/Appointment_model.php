...
class Appointment_model extends CI_Model {
	...
	public function read()
	{
		return $this->db->select("
				...
				user.lastname,  
				department.name, 
				CASE WHEN pr_prescription.id THEN 'bg-success' ELSE NULL END AS isPrescribe,
				pr_prescription.id AS prescription_id
			")
			->from($this->table)
			->join('user','user.user_id = appointment.doctor_id')
			->join('department','department.dprt_id = appointment.department_id') 
			->join('pr_prescription','pr_prescription.appointment_id = appointment.appointment_id', 'left') 
